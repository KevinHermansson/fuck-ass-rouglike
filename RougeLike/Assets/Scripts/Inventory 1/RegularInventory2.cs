...
public class RegularInventory2 : MonoBehaviour
{
    public const int Capacity = 9;
    public static RegularInventory2 Instance { get; private set; }
    ...
    {
        if (item == null || item.Category != ItemCategory.Item) return false;
    ...
    {
        if (index < 0 || index >= Capacity) return;
        if (newItem == null || newItem.Category != ItemCategory.Item) return;
        slots[index] = newItem;
        OnChanged?.Invoke();
    }
