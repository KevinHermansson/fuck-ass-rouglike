...
public class SeedInventory2 : MonoBehaviour
{
    public const int Capacity = 6;
    public static SeedInventory2 Instance { get; private set; }
    ...
    {
        if (item == null || item.Category != ItemCategory.Seed) return false;
    ...
    {
        if (index < 0 || index >= Capacity) return;
        if (newItem == null || newItem.Category != ItemCategory.Seed) return;
        slots[index] = newItem;
        OnChanged?.Invoke();
    }
